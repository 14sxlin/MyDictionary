...
//    @Ignore
    test("format word"){
      val queryContent = QEWord("apple")
      val source = ConnectionTool.queryFromWebsite(queryContent)
      val result = ResultFormatter.formatWord(source).asInstanceOf[Word]
      assert(("n.","苹果；苹果树；苹果公司；")==result.type_mean(0))
      assert(2 == result.sound.size)
      assert(result.pron == "英 [ˈæpl] 美 [ˈæpəl]")
//      print(result.pron)
      val queryContent1= QEWord("do")
      val source1 = ConnectionTool.queryFromWebsite(queryContent1)
      val result1 = ResultFormatter.formatWord(source1).asInstanceOf[Word]
...
//      print(text)
  ...
  /**
    ...
    */
  test("查询英文句子"){
    val ephrase = ContentWrapper.wrap("in what case")
    val queryContent = ConnectionTool.queryFromWebsite(ephrase)
  ...
  /**
    ...
    */
  test("查询英语句子 类型1"){
    val eSentence = ContentWrapper.wrap("I love you")
    val source = ConnectionTool.queryFromWebsite(eSentence)
    val sentence =  ResultFormatter.formatSentence1(eSentence)
    assert(sentence.meaning.trim == "n. 我爱你；".trim)
  }

  test("查询英语短语"){
    val eSentence = ContentWrapper.wrap("holy shit")
    val source = ConnectionTool.queryFromWebsite(eSentence)
    val sentence =  ResultFormatter.formatSentence1(eSentence)
  ...
  /**
    ...
    */
  test("查询中文句子"){
    val eSentence = ContentWrapper.wrap("我爱你")
    val source = ConnectionTool.queryFromWebsite(eSentence)
