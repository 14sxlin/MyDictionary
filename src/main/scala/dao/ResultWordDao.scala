...
import dao.WordDao.queryOneWord
...
object ResultWordDao {
  ...
  def queryOneResult(english:String): Option[ResultWord] = {
    ...
    val result = {
      queryOneWord(english) match {
        case None => null
        case Some(word) =>
          val typeMeans = TypeMeanDao.queryTypeMeansByWordId(word.getId)
  ...
  private def saveWordAndTypeMean(resultWord:ResultWord): Unit = {
    DAOUtils.doInSessionAndCommit { wordMapper: WordMapper =>
      ...
      TypeMeanDao.saveTypeMeans(word.getId,resultWordGetTypeMeans(resultWord))
    }
  }
